...
class ArvBin {
	...
	private:
	    ...
	    int calcularImparesNasFolhas(NoArv* p);
...
int ArvBin::calcularImparesNasFolhas() {
	
    return calcularImparesNasFolhas(raiz);
...
int ArvBin::calcularImparesNasFolhas(NoArv* p) {
    ...
    if (p == NULL) {
        ...
        return (p->getInfo() % 2 != 0) ? 1 : 0;
        
    } 
    
	else {
    	
        return calcularImparesNasFolhas(p->getEsq()) + calcularImparesNasFolhas(p->getDir());
...
int main() {
    ...
    int imparesNasFolhas = minhaArvore.calcularImparesNasFolhas();
    ...
    std::cout << "A quantidade de nos folhas com numeros impares na arvore e: " << imparesNasFolhas << std::endl;
	return 0;
